...
If you want a spin-1 term, add "1" to the end of the tuple: {"A",1.2,0.7,1}. Anything other than 1 added at the end is ignored;
This of course is substantially more effort than computing a spin-1/2 term;

Example uses: Calculation[6,6,0,{{"A",1.2,0.7},{"A",1,0},{"B",1.7,1.2}},0,10^6,False];
			Calculation[12,12,1,{{"A",1,1},{"B",1,0,1}},1/(6*10^-6),False];
...
Hamiltonian[\[Omega]_,ihfca_,\[CapitalDelta]Aa_,ihfcb_,\[CapitalDelta]Ab_,m_,n_,i_,j_]:=(
...
HFI=HFImT;
mn=m*n; 
{\[Omega],HFI[[All,{2,3}]]}={\[Omega]mT,HFImT[[All,{2,3}]]}*mT2angfreq;
...
Lengths=Table[Length[Ahfc[[j]]]>3,{j,1,Length[Ahfc]}]; (*tabulate which are longer than 4*)
Length4s=Position[Lengths,True]//Flatten;
Na1=Count[Ahfc[[Length4s]][[All,4]],1];(*see how many have "1" in the 4th entry*)
Na=Na-Na1;
Spin1sinLength4s=Position[Ahfc[[Length4s]][[All,4]],1]//Flatten; (*Find position **in Length4s** of spin 1s*)
...
Lengths=Table[Length[Bhfc[[j]]]>3,{j,1,Length[Bhfc]}];
Length4s=Position[Lengths,True]//Flatten;
Nb1=Count[Bhfc[[Length4s]][[All,4]],1];
Nb=Nb-Nb1;
Spin1sinLength4s=Position[Bhfc[[Length4s]][[All,4]],1]//Flatten;
...
Hamiltonian[\[Omega],ihfca,\[CapitalDelta]Aa,ihfcb,\[CapitalDelta]Ab,m,n,i,j];
